...
from flcore.clients.clientcross_rul import clientCross_RUL
import torch.nn.functional as F

class FedCross_RUL(Server_RUL):
    def __init__(self, args, times):
        super().__init__(args, times)
        
        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientCross_RUL)
    ...
    def train(self):
        ...
        if self.num_new_clients > 0:
            ...
            self.set_new_clients(clientCross_RUL)
            print(f"\n-------------Fine tuning round-------------")
